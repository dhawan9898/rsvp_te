 ...
 * @brief Linux-specific timer hardware abstraction layer.
 * @details Implements the timer hardware abstraction layer using Linux timerfd.
 */
...
int hal_timer_init(void) {
    /* Create a non-blocking monotonic timer file descriptor */
...
void hal_timer_set(uint32_t timeout_ms) {
    ...
    /* Calculate timeout values in seconds and nanoseconds */
    new_value.it_value.tv_sec = timeout_ms / 1000;
    new_value.it_value.tv_nsec = (timeout_ms % 1000) * 1000000;
    /* This is a one-shot timer, so interval is set to zero */
    new_value.it_interval.tv_sec = 0;
    new_value.it_interval.tv_nsec = 0;

    /* Arm the timer with the calculated values */
...
void hal_timer_clear(void) {
    ...
    /* Setting timer values to zero disarms the timer */
    if (timerfd_settime(central_tfd, 0, &new_value, NULL) < 0) {
