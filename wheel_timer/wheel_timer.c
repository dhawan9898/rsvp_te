...
struct timer_wheel {
    ...
    pthread_mutex_t wait_lock; /* Protects executing state changes for sync_del */
...
static void internal_add_timer(timer_wheel_t *wheel, timer_node_t *node) {
    ...
    if (idx <= 0) {
        ...
        int i = (expires >> (WHEEL_TVR_BITS + 2 * WHEEL_TVN_BITS)) & WHEEL_TVN_MASK;
        vec = wheel->tv4 + i;
    } else {
        int i = (expires >> (WHEEL_TVR_BITS + 3 * WHEEL_TVN_BITS)) & WHEEL_TVN_MASK;
        vec = wheel->tv5 + i;
    }
...
static void *worker_thread_func(void *arg) {
    ...
    while (1) {
        ...
        while (atomic_load(&wheel->running) && list_empty(&wheel->expired_list)) {
        ...
        list_for_each_safe(pos, tmp, &node_list) {
            ...
            if (atomic_load(&wheel->running) && node->period > 0 && !node->pending) {
...
static void *tick_thread_func(void *arg) {
    ...
    while (atomic_load(&wheel->running)) {
        ...
        if (!index &&
            (!cascade(wheel, wheel->tv2, (next_jiffies >> WHEEL_TVR_BITS) & WHEEL_TVN_MASK)) &&
            (!cascade(wheel, wheel->tv3, (next_jiffies >> (WHEEL_TVR_BITS + WHEEL_TVN_BITS)) & WHEEL_TVN_MASK)) &&
            (!cascade(wheel, wheel->tv4, (next_jiffies >> (WHEEL_TVR_BITS + 2 * WHEEL_TVN_BITS)) & WHEEL_TVN_MASK))) {
            cascade(wheel, wheel->tv5, (next_jiffies >> (WHEEL_TVR_BITS + 3 * WHEEL_TVN_BITS)) & WHEEL_TVN_MASK);
        }
...
timer_wheel_t* timer_wheel_create(unsigned int tick_ms, int worker_count) {
    ...
    if (pthread_mutex_init(&wheel->dispatch_lock, NULL) != 0) goto err_destroy_lock;
    if (pthread_mutex_init(&wheel->wait_lock, NULL) != 0) goto err_destroy_dispatch_lock;
    if (pthread_cond_init(&wheel->dispatch_cond, NULL) != 0) goto err_destroy_wait_lock;
    if (pthread_cond_init(&wheel->wait_cond, NULL) != 0) goto err_destroy_dispatch_cond;
    ...
    for (int i = 0; i < worker_count; i++) {
        ...
        if (pthread_create(&wheel->workers[i], NULL, worker_thread_func, wargs) != 0) {
            free(wargs);
    ...
    if (pthread_create(&wheel->tick_thread, NULL, tick_thread_func, wheel) != 0) {
        goto err_stop_workers;
    }
